...
class APIConnectorImpl implements APIConnector {
                         ...
                         $headers, $responseType)
    {
        ...
        $http_headers = [];
        // Parsing headers to match the output of CURL's client implementation of parse_headers()
        foreach ($response->getHeaders() as $name => $values) {
            $http_headers[$name] = count($values) == 1 ? $values[0] : $values;
        }
        ...
        if ($response->getStatusCode() == 0) {
            ...
            if ($responseType == '\SplFileObject') {
                return array($http_body, $response->getStatusCode(), $http_headers);
            }
